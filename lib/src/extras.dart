...
Future<String?> getTokenForRealdebrid(
    ({String username, String password}) private) async {
  String payload;
  URLRequest startup;
  ...
  await Future.delayed(const Duration(seconds: 6));
  payload =
      "document.querySelector('#loginform > fieldset:nth-child(1) > input[type=text]').value = '${private.username}';";
  await browser.webViewController?.evaluateJavascript(source: payload);
  await Future.delayed(const Duration(seconds: 2));
  payload =
      "document.querySelector('#loginform > fieldset:nth-child(3) > input[type=password]').value = '${private.password}';";
  await browser.webViewController?.evaluateJavascript(source: payload);
  await Future.delayed(const Duration(seconds: 2));
  ...
  final invalid = await browser.webViewController
      ?.evaluateJavascript(source: payload) as bool;
  if (invalid) return null;
  startup = URLRequest(url: WebUri('https://real-debrid.com/apitoken'));
  await browser.webViewController?.loadUrl(urlRequest: startup);
  await Future.delayed(const Duration(seconds: 6));
  payload =
      "document.querySelector('#wrapper_global > div > div > input[type=text]').value;";
  final results =
      await browser.webViewController?.evaluateJavascript(source: payload);
  return results;
}
...
Future<bool> setPairForRealdebrid(
    ({String username, String password}) private, String pincode) async {
  ...
  payload =
      "document.querySelector('body > div > div > div:nth-child(2) > div > div.panel-body > form > fieldset > div.pull-right > input').click();";
  await browser.webViewController?.evaluateJavascript(source: payload);
  await Future.delayed(const Duration(seconds: 4));
  payload =
      "document.querySelector('body > div > div > div:nth-child(2) > div > div.panel-body > form > fieldset > input.btn.btn-lg.btn-success.btn-block').click();";
  await browser.webViewController?.evaluateJavascript(source: payload);
  await Future.delayed(const Duration(seconds: 6));
  return true;
}
...
Future<bool> setPairForTrakt(
  ...
  await Future.delayed(const Duration(seconds: 6));
  payload =
      "document.querySelector('#user_login').value = '${private.username}';";
  await browser.webViewController?.evaluateJavascript(source: payload);
  await Future.delayed(const Duration(seconds: 2));
  payload =
      "document.querySelector('#user_password').value = '${private.password}';";
  await browser.webViewController?.evaluateJavascript(source: payload);
  await Future.delayed(const Duration(seconds: 2));
  payload =
      "document.querySelector('#new_user > div.form-actions > input').click();";
  await browser.webViewController?.evaluateJavascript(source: payload);
  ...
  await Future.delayed(const Duration(seconds: 2));
  payload =
      "document.querySelector('#auth-form-wrapper > form > div.form-actions > input').click();";
  await browser.webViewController?.evaluateJavascript(source: payload);
  await Future.delayed(const Duration(seconds: 12));
  address = (await browser.webViewController?.getUrl()).toString();
